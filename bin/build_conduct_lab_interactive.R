...
inject_interactivity <- function(html) {
  ...
  js <- c(
    ...
    '    if(!el) return false;',
    '    if(el.matches && el.matches("div.sourceCode, pre.sourceCode")) return true;',
    '    if(el.tagName === "PRE" && el.querySelector("code[class*=language-], code.r, code.stata")) return true;',
    '    return false;',
    '  }',
    ...
    '  }',
    '  function wire(){',
    '    const blocks = Array.from(document.querySelectorAll("div.sourceCode, pre.sourceCode"));',
    '    blocks.forEach((block)=>{',
    '      if(block.dataset.interactiveWired === "1") return;',
    '      const target = findPlotTarget(block);',
    '      if(!target){ block.dataset.interactiveWired = "1"; return; }',
