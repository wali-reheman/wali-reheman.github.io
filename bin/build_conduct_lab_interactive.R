...
inject_interactivity <- function(html) {
  ...
  js <- c(
    ...
    '(function(){',
    '  const state = { webR: null, webRReady: null, installedPkgs: new Set() };',
    '',
    ...
    '    const pkgs = [];',
    '    const re = /(?:library|require)\\s*\\(\\s*["\\\']?([A-Za-z][A-Za-z0-9._]*)["\\\']?.*?\\)/g;',
    '    let m;',
    '    while((m = re.exec(code)) !== null){',
    '      if(m[1]) pkgs.push(m[1]);',
    '    }',
    ...
    '        out.push("dplyr", "tidyr", "ggplot2", "magrittr");',
    '      } else {',
    '        out.push(name);',
    '      }',
    '    });',
    ...
    '    const wanted = normalizePackages(extractPackages(code)).filter((p)=>!state.installedPkgs.has(p));',
    ...
    '        `  tryCatch(install.packages(\'${pkg}\', quiet = TRUE), error = function(e) NULL)` ,',
    '        `}`,',
