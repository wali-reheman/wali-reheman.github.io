...
to_slide_lines <- function(lines) {
  ...
  for (line in lines) {
    ...
    if (!in_code) {
      # Avoid accidental slide breaks from markdown horizontal rules.
      if (grepl('^\\s*-{3,}\\s*$', line)) {
        line <- ''
      }
    }

    if (!in_code && grepl('^#{1,2}\\s+', line)) {
...
build_header <- function(title, roadmap, workdir_rel) {
  c(
    ...
    '  .remark-slide-content { font-size: 18px; padding: 0.75em 0.95em; }',
    '}',
    '@media (max-width: 800px) {',
    '  .remark-slide-content { font-size: 16px; padding: 0.65em 0.8em; }',
    '  .remark-slide-content pre { font-size: 0.68em; }',
    '}',
    '</style>',
    '',
    'class: center, middle',
