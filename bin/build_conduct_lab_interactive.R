...
inject_interactivity <- function(html) {
  css <- c(
    ...
    ':root {',
    '  --course-accent: #0f4d92;',
    '  --course-accent-soft: #e8f1fd;',
    '  --course-ink: #1f2a38;',
    '  --course-muted: #5a6b80;',
    '}',
    'body {',
    '  background:',
    '    radial-gradient(920px 320px at 0% 0%, rgba(15,77,146,0.09), transparent 58%),',
    '    radial-gradient(900px 300px at 100% 0%, rgba(15,77,146,0.07), transparent 60%);',
    '}',
    '.main-container {',
    '  max-width: 1140px;',
    '  margin-top: 0.9rem;',
    ...
    '  border-radius: 14px;',
    '  background: #fff;',
    '  box-shadow: 0 12px 28px rgba(16, 46, 89, 0.07);',
    '}',
    '.main-container h1, .main-container h2, .main-container h3 {',
    '  color: var(--course-ink);',
    '}',
    '.main-container h1 { margin-bottom: 0.95rem; }',
    '.main-container h2 {',
    '  margin-top: 1.3rem;',
    ...
    '}',
    '.sourceCode, pre[class], pre {',
    '  cursor: pointer;',
    '  border-left: 4px solid #d4e4fb;',
    '  border-radius: 10px;',
    '  background: #f9fbff;',
    '  box-shadow: inset 0 0 0 1px #e3edf9;',
    '  margin-bottom: 0.45rem;',
    '}',
    '.sourceCode code, pre code { font-size: 0.84rem; line-height: 1.45; }',
    '.code-toolbar {',
    '  display: flex;',
    '  flex-wrap: wrap;',
    '  gap: 0.4rem;',
    ...
    '}',
    '.code-btn:hover { background: #e8f2ff; border-color: #9ec0ee; }',
    '.code-btn:disabled { opacity: 0.65; cursor: not-allowed; }',
    '.code-btn.run-btn {',
    '  background: var(--course-accent);',
    '  border-color: var(--course-accent);',
    '  color: #fff;',
    '}',
    '.code-btn.run-btn:hover { filter: brightness(0.95); }',
    '.code-editor {',
    '  width: 100%;',
    '  min-height: 160px;',
    ...
    '  transition: all 120ms ease;',
    '}',
    '.plot-toggle-btn:hover { background: #e8f2ff; border-color: #9ec0ee; }',
    ...
    '}',
    '.table-responsive, .pagedtable-wrapper {',
    ...
    '  border-radius: 8px;',
    '}',
    'pre, .sourceCode, .code-output { overflow-x: auto; }',
    'a { color: var(--course-accent); }',
    'a:hover { color: #0a3d74; }',
    ...
    '    border-radius: 10px;',
    '  }',
    '  .figure img, p > img { max-height: 56vh !important; }',
    '  .code-editor { min-height: 138px; }',
    '}',
    '</style>'
  )
  ...
  js <- c(
    ...
    '              ".__coursehub_vis <- withVisible({\\n" + code + "\\n})\\n" +',
    '              "if (isTRUE(.__coursehub_vis$visible)) {\\n" +',
    '              "  .__coursehub_res <- .__coursehub_vis$value\\n" +',
    '              "  if (inherits(.__coursehub_res, \'data.frame\')) {\\n" +',
    '              "    .n <- NROW(.__coursehub_res); .k <- NCOL(.__coursehub_res)\\n" +',
    '              "    if (.n > 12) {\\n" +',
    '              "      cat(sprintf(\'[showing first 12 of %d rows x %d columns]\\\\n\', .n, .k))\\n" +',
    '              "      print(utils::head(.__coursehub_res, 12))\\n" +',
    '              "    } else {\\n" +',
    '              "      print(.__coursehub_res)\\n" +',
    '              "    }\\n" +',
    '              "  } else if (is.matrix(.__coursehub_res)) {\\n" +',
    '              "    .n <- NROW(.__coursehub_res); .k <- NCOL(.__coursehub_res)\\n" +',
    '              "    if (.n > 12) {\\n" +',
    '              "      cat(sprintf(\'[showing first 12 of %d rows x %d columns]\\\\n\', .n, .k))\\n" +',
    '              "      print(utils::head(.__coursehub_res, 12))\\n" +',
    ...
    '              "      print(.__coursehub_res)\\n" +',
    '              "    }\\n" +',
    '              "  } else if (is.atomic(.__coursehub_res) && length(.__coursehub_res) > 80) {\\n" +',
    '              "    cat(sprintf(\'[showing first 80 of %d values]\\\\n\', length(.__coursehub_res)))\\n" +',
    '              "    print(utils::head(.__coursehub_res, 80))\\n" +',
    '              "  } else {\\n" +',
    '              "    print(.__coursehub_res)\\n" +',
    '              "  }\\n" +',
    '              "}\\n" +',
    ...
    '            const rendered = clean && !/^\\[object RObject:/.test(clean) ? clean : "Code executed.";',
    '            const lines = rendered.split(/\\r?\\n/);',
    '            if (rendered.length > 1200 || lines.length > 25) {',
    '              const clipped = lines.slice(0, 25).join("\\n").slice(0, 1200);',
